...
def test_get_authors_no_records(client):
    ...
    expected_result = {
        ...
        'pagination': {
            'total_pages': 0,
            'total_records': 0,
            'current_pages': '/api/v1/authors?page=1'
        }
    }
...
def test_get_authors_with_params(client, sample_data):
    ...
    assert response_data['pagination'] == {
        'total_pages': 5,
        'total_records': 10,
        'current_pages': '/api/v1/authors?page=2&fields=first_name&sort=-id&limit=2',
...
@pytest.mark.parametrize(
    'data, missing_field',
    [
        ({'last_name': 'Mickiewicz', 'birth_date': '24-12-1898'}, 'first_name'),
        ({'first_name': 'Adam', 'birth_date': '24-12-1898'}, 'last_name'),
